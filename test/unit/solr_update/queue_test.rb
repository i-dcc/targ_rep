...
class SolrUpdate::QueueTest < ActiveSupport::TestCase
  context 'SolrUpdate::Queue' do
    ...
    should 'be run and process items in order they were added' do
      ...
      SolrUpdate::Queue::Item.expects(:process_in_order).with().multiple_yields([object1, 'update'], [object2, 'delete'])

      SolrUpdate::CommandFactory.expects(:create_solr_command_to_update_in_index).with(object1).returns(command1)
      SolrUpdate::CommandFactory.expects(:create_solr_command_to_delete_from_index).with(object2).returns(command2)
